...
class AuthController {
    ...
    async login(req, res) {
        try {
            ...
            // В MongoDB поле может называться is_blocked (как в схеме)
            if (user.is_blocked) {
                return res.status(403).json({ error: "User is blocked" });
            }

            // 4. Сравнение паролей
            // user.hashed_password должно существовать, так как мы не исключали его в findByEmail
            ...
            // 5. Генерация токенов
            // Важно: user.id должен быть строкой. Наш репозиторий делает маппинг _id -> id
            const accessToken = this.generateAccessToken(user);
            const refreshToken = this.generateRefreshToken(user);

            // 6. Сохранение refresh токена
            // user.id уже строка благодаря маппингу в репозитории
            await this.tokenRepo.save(refreshToken, user.id);

            res.json({ accessToken, refreshToken });
        } catch (err) {
            console.error('Login Error:', err);
            // Отправляем детальную ошибку только в режиме разработки, в продакшене лучше generic message
